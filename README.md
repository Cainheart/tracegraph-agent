...
- TS/JS file/module/static import/export Graph Snapshot 与 Delta，并额外抽取顶层 AST `class/enum/function/interface/namespace/type/variable` symbol 与 `contains` 关系；它不是动态调用、DI、路由、方法级或跨语言调用图；
...
- G-12 提供 Host-owned 原生 LSP stdio 客户端：按项目懒启动 `typescript-language-server`/`pyright-langserver` 候选，完成 `initialize`、`didOpen/didChange`、`publishDiagnostics`、`definition`、`references`；`get_diagnostics` 走标准 Tool/Receipt/Observation 路径，完整诊断只作为 bounded observation，Ledger/Projection 只保存 `lsp.diagnostics_received` 摘要和 `lsp.server_unavailable` 降级事实。Host/SDK 的 `/api/lsp` 与 Web Settings 只显示 server 状态/诊断计数；没有可用 server 时返回可审计的 unavailable，不伪造空诊断；G-20 会复用已记录的有界摘要到 `code_intel`，但自动 Context 注入、完整索引和自动诊断仍未实现；
- G-20 将静态 CodeGraph、G-12 摘要和可审计 Git 基线接到 Run 事实链：Run baseline/成功 Patch 后分别写 `code.intel_updated`，Changes 视图显示有界 changed files/顶层 symbols/LSP 摘要；标准 CLI composition 用固定只读 Git probe 记录 `HEAD`、branch、dirty 和包含 untracked 文件的 status fingerprint。若先前可用的 Git 基线在 Patch ask 审批前变化，旧 approval 会以 `stale_base` 作废并创建新 approval，不能凭旧 token 写盘；policy `allow` 遇到同样漂移也会降级为新的手工审批。Git 不可用或未注入自定义 composition 时不宣称这个 fence；
