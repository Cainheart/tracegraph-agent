...
| 子模块 | 评审焦点 |
|---|---|
| [AGENTS、Notes 与 Skills](01-agents-notes-skills.md) | 规则、理由和可复用操作如何各守一份真源 |
...
Skill 是可复用操作手册，适合稳定且多次重复的仓库流程。当前已编写的中英文双入口及实际能力边界，以[仓库 Skill 清单](../../../.agents/skills/README.md)为准；下表仍是 V2 目标候选，不表示 Snapshot、完整 Benchmark、翻译 hash 或公开证明流程已落地：
...
可交给实现者的任务卡、证据矩阵和规则维护循环见[工程研发与维护 SOP](04-engineering-sop.md)；本节只定义仓库级顺序。
