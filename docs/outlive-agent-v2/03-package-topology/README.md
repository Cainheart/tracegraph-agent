...
```mermaid
flowchart LR
  F[Foundation] --> T[Evidence · Session]
  T --> R[Runtime · Tool · Context · LLM]
  R --> M[Memory · Execution · Integration]
  M --> O[Orchestration · Control · Client]
  P[Profiles & Gates] -.校验装配.-> O
  P -.守住依赖.-> R
```

| 子模块 | 覆盖家族 |
|---|---|
| [Foundation、Evidence 与 Session](01-foundation-evidence-session.md) | 基础契约、事实账本、会话持久格式 |
| [Runtime、Tool、Context 与 LLM](02-runtime-tool-context-llm.md) | 执行循环及其四个核心接缝 |
| [Memory、Execution 与 Integration](03-memory-execution-integration.md) | 长期知识、本地执行与外部协议 |
| [Orchestration、Control 与 Client](04-orchestration-control-client.md) | 多任务编排、控制器、共享客户端投影 |
| [Profiles、依赖门禁与升包](05-profiles-dependency-gates.md) | 组合根、允许依赖和物理拆包决定 |
...
因此，[迁移基线](../10-tracegraph-to-outlive-migration/README.md)保留的“先在 core 内分层”仍是正确第一步；本文件同时明确：边界稳定并满足升包门槛后，可以物理拆包。
...
第一阶段不必物理创建 `brand/time/bounded` 包，可作为 `contracts`/`core/kernel` 内模块；即使出现两个以上消费者，仍需稳定边界与 contract tests 才能升包。
